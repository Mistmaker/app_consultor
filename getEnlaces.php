<?php
    ...
    try {
        ...
        $database->query("SELECT * FROM grupo_enlace WHERE id IN (SELECT ID_GRUPO FROM enlaces) ORDER BY id");
        ...
        for ($x = 0; $x < count($rows); $x++)
        {
            ...
            $database = new Database();
            // $encabezado = $rows[$x];
            $database->query("SELECT * FROM enlaces WHERE ID_GRUPO =".$rows[$x]["id"]);
